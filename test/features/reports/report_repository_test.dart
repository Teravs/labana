...
void main() {
  ...
  group('Laporan Harian (Daily)', () {
    test(
      'Empty state ketika tidak ada transaksi pada tanggal tersebut',
      () async {
        final summary = await reportRepo.getReportSummary(
          '2026-09-16',
          '2026-09-16',
        );
        ...
        final breakdown = await reportRepo.getProductBreakdown(
          '2026-09-16',
          '2026-09-16',
        );
        expect(breakdown, isEmpty);

        final top = await reportRepo.getTopSellingProduct(
          '2026-09-16',
          '2026-09-16',
        );
        expect(top, isNull);

        final highest = await reportRepo.getHighestProfitProduct(
          '2026-09-16',
          '2026-09-16',
        );
    ...
    test(
      'Satu transaksi menghasilkan angka omzet, HPP, laba, dan produk terjual yang akurat',
      () async {
    ...
    test(
      'Beberapa transaksi di hari yang sama terakumulasi dan mengisolasi tanggal lain',
      () async {
        final p1 = await insertProduct('Es Teh Manis');
        final r1 = await insertRecipeVersion(p1, 2000);
  ...
  group('Laporan Mingguan (Weekly)', () {
    test(
      'Mengakumulasikan transaksi Senin–Minggu dan mengabaikan minggu sebelum/sesudah',
      () async {
    ...
    test(
      'getDailyBreakdown menyusun 7 hari lengkap (Senin–Minggu) dengan tanggal kosong Rp0',
      () async {
        ...
        final dailyBreakdown = await reportRepo.getDailyBreakdown(
          '2026-09-14',
          '2026-09-20',
        );
  ...
  group('Laporan Bulanan (Monthly)', () {
    test(
      'Mengakumulasikan transaksi dalam satu bulan dan mengisolasi bulan sebelum/sesudah',
      () async {
    ...
    test(
      'Bulan Februari tahun kabisat (2024: 29 hari) tercover hingga tanggal 29',
      () async {
  ...
  group('Breakdown Produk & Deterministic Tie-Breaker', () {
    test(
      'Produk yang sama diakumulasi di seluruh transaksi dalam periode',
      () async {
        final p1 = await insertProduct('Es Teh Manis');
        final r1 = await insertRecipeVersion(p1, 2000);
    ...
    test(
      'Top Selling & Highest Profit dengan deterministic tie-breaker (product_id ASC)',
      () async {
        ...
        final topSelling = await reportRepo.getTopSellingProduct(
          '2026-09-16',
          '2026-09-16',
        );
        ...
        final highestProfit = await reportRepo.getHighestProfitProduct(
          '2026-09-16',
          '2026-09-16',
        );
  ...
  group('Historical Snapshot Integrity', () {
    test(
      'Perubahan master data harga, nama, atau resep TIDAK mengubah data laporan transaksi lama',
      () async {
        ...
        final initialSummary = await reportRepo.getReportSummary(
          '2026-09-10',
          '2026-09-10',
        );
        ...
        final postSummary = await reportRepo.getReportSummary(
          '2026-09-10',
          '2026-09-10',
        );
        ...
        final postBreakdown = await reportRepo.getProductBreakdown(
          '2026-09-10',
          '2026-09-10',
        );
        expect(
          postBreakdown.first.productName,
          'Es Lemon Tea Original',
        ); // Nama snapshot tetap terjaga
  ...
  group('getReportData Facade Integration', () {
    test(
      'Memuat seluruh data (summary, products, top, highest, daily, paymentMethods) secara konsisten',
      () async {
